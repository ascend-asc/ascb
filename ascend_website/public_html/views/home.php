...
<?php
$page_title = 'Home';
$page_description = 'Andres Soriano Colleges of Bislig (ASCB) is a leading private educational institution offering quality basic, technical-vocational, and higher education programs in Bislig City, Surigao del Sur.';
?>
        ...
        <div class="row g-5 mb-5">
            <div class="col-md-6">
                <div class="vm-card vision">
                    <div class="vm-icon">
                        <i data-lucide="telescope" style="width: 36px; height: 36px;"></i>
                    </div>
                    <h3>VISION</h3>
                    <p><?php echo nl2br(htmlspecialchars($identity->vision ?? 'ASCB envisions itself as a leading private educational institution in the region and beyond, fostering an empowering and transformative education that develops globally competent, values-driven, and socially engaged individuals.')); ?></p>
                </div>
            </div>
            <div class="col-md-6">
                <div class="vm-card mission">
                    <div class="vm-icon">
                        <i data-lucide="mountain-snow" style="width: 36px; height: 36px;"></i>
                    </div>
                    <h3>MISSION</h3>
                    <p><?php echo nl2br(htmlspecialchars($identity->mission ?? 'ASCB commits itself to delivering quality, inclusive, and innovative education that nurtures academic excellence, character, creativity, leadership, and service, while preparing learners to thrive in a dynamic global community.')); ?></p>
                </div>
            </div>
        </div>

        <div class="section-divider">
            <div class="section-divider-title">CORE VALUES</div>
        </div>

        <?php 
        $cv_classes = [
            'A' => ['card_class' => 'cv-a', 'icon' => 'shield-check'],
            'S' => ['card_class' => 'cv-s', 'icon' => 'sprout'],
            'C' => ['card_class' => 'cv-c', 'icon' => 'heart-handshake'],
            'B' => ['card_class' => 'cv-b', 'icon' => 'lightbulb']
        ];
        ?>
        <div class="row g-4 mt-4">
            <?php foreach(['A', 'S', 'C', 'B'] as $letter): ?>
            <div class="col-md-3">
                <div class="cv-card <?php echo $cv_classes[$letter]['card_class']; ?>">
                    <div class="cv-letter-circle">
                        <?php echo $letter; ?>
                    </div>
                    <h4><?php echo htmlspecialchars($cv[$letter]['label'] ?? "{$letter} - Value"); ?></h4>
                    <p><?php echo htmlspecialchars($cv[$letter]['desc'] ?? 'Value description goes here.'); ?></p>
                    <div class="cv-bottom-ribbon">
                        <i data-lucide="<?php echo $cv_classes[$letter]['icon']; ?>" style="width: 28px; height: 28px;"></i>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
